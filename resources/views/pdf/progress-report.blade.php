...
<body>
    <div class="header">
        <h1>Bao cao tien do chinh ly</h1>
        <div class="subtitle">Che do: {{ $modeLabel }}</div>
        @if ($dateFrom || $dateTo)
            <div class="subtitle">
                @if ($dateFrom) Tu {{ \Carbon\Carbon::parse($dateFrom)->format('d/m/Y') }} @endif
                @if ($dateTo) den {{ \Carbon\Carbon::parse($dateTo)->format('d/m/Y') }} @endif
            </div>
        @endif
    </div>
    ...
    <table>
        <thead>
            <tr>
                <th style="text-align: left;">Moc thong ke</th>
                <th>Ho so chinh ly</th>
                <th>Tai lieu</th>
            </tr>
        </thead>
        <tbody>
            ...
            @empty
                <tr class="empty-row">
                    <td colspan="3">Khong co du lieu phu hop</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
